...
export default function SliderPadrao({ cursos }) {
    ...
    return (
        <>
            ...
            <div className={`fixed lg:p-10 top-0 left-0 bottom-0 right-0 w-full h-full flex items-end lg:items-center lg:justify-center ${modalOpen ? '' : 'hidden'} backdrop-blur`}>
                <div className="bg-zinc-900 rounded-lg p-6 relative min-w-[50%] max-w-3xl modal-animation-fade">
                    <button onClick={() => setOpen(!modalOpen)} className="absolute top-5 right-5 text-white">
                        <X size={30} weight="bold" className="" />
                    </button>
                    <div className="flex gap-x-3 lg:gap-x-10 py-5">
                        ...
                        <div className="flex flex-col w-full">
                            <h2 className="text-lg lg:text-4xl text-white mb-3">{info?.title}</h2>
                            <p className="text-white opacity-75 text-sm xl:text-lg mb-8">{info?.description}</p>
                            <Link href={`/${info?.link}`}>
                                <a>
